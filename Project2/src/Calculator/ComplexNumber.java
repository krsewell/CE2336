...
public class ComplexNumber extends Number {
  ...
  public String toString() {
    if (this.get_real_number() == 0.0) {
      return String.format("%.2f", this._imaginary_number) + (this._imaginary_number == 0.0 ? "" : "i");
    } else if (this.get_imaginary_number() == 0.0) {
      return super.toString();
    } else {
      return super.toString() + (this._imaginary_number > 0.0 ? "+" : "-") +
              String.format("%.2f", Math.abs(this._imaginary_number)) + "i";
    }
  }

  @Override
  public boolean equals(Object obj) {
    return obj instanceof ComplexNumber && (super.equals(obj) && this._imaginary_number == ((ComplexNumber) obj).get_imaginary_number());
